...
import { Piece } from "./piece-tree-base";
...
export function leftRotate(tree: ITree, x: TreeNode) {
  const y = x.right;
...
export function rightRotate(tree: ITree, y: TreeNode) {
  const x = y.left;
  y.left = x.right;
  if (x.right !== SENTINEL) {
    x.right.parent = y;
...
export function rbDelete(tree: ITree, z: TreeNode) {
  let x: TreeNode;
  let y: TreeNode;
...
export function fixInsert(tree: ITree, x: TreeNode) {
  recomputeTreeMetadata(tree, x);
...
export function recomputeTreeMetadata(tree: ITree, x: TreeNode) {
  let delta = 0;
  let lf_delta = 0;
  if (x === tree.root) {
    return;
